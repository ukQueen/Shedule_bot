...
cancel_kb = ReplyKeyboardMarkup(keyboard=[[KeyboardButton(text="✖️Отмена")]],
                                resize_keyboard=True,
                                input_field_placeholder="Choose the button")

geo_kb = ReplyKeyboardMarkup(keyboard=[[KeyboardButton(text="📍Отправить местоположение", request_location=True)],
                                       [KeyboardButton(text="✖️Отмена")]],
                             resize_keyboard=True,
                             input_field_placeholder="Send your geolocation")


first_kb = ReplyKeyboardMarkup(keyboard=[[KeyboardButton(text='Авторизоваться'), KeyboardButton(text='Отметиться')],
...
def get_education_kb():
    ...
    buttons_buf = [[InlineKeyboardButton(text="Бакалавриат", callback_data="education_бакалавр")],
...
def get_institute_kb(education, page):
    ...
    buttons_buf = [[InlineKeyboardButton(text=el['Institute'],
    ...
    end_buttons = []
    if page > 1:
        end_buttons.append(InlineKeyboardButton(text="⬅️",
                                                callback_data="institute_page_" + str(page - 1)))
...
def get_fuclty_kb(institute, education, page):
    ...
    if not len(fuclty) % 7 == 0:
        pages += 1
    buttons = [InlineKeyboardButton(text="✖️ Отмена", callback_data="undo")]
    button_back = [InlineKeyboardButton(text="🔙 Назад", callback_data="education_back")]

    group_buttons = [[InlineKeyboardButton(text=el['Fuclty'],
                                           callback_data="id_fuclty_" + str(el['id_fuclty']))]
                     for el in fuclty]
    start_index = (page - 1) * 7
    end_index = page * 7
    ...
    if page > 1:
        end_buttons.append(InlineKeyboardButton(text="⬅️",
                                                callback_data="fuclty_page_" + str(page - 1)))
...
def get_group_kb(institute, course, education,page):
    ...
    buttons = []
    if course > 1:
        buttons.append(InlineKeyboardButton(text=str(course - 1) + " курс",
    ...
    button_back = [InlineKeyboardButton(text="🔙 Назад", callback_data="id_institute_back")]
    group_buttons = [InlineKeyboardButton(text=el['Study_group'],
                                          callback_data="id_study_group_" + str(el['id_study_group']))
                     for el in group]
    group_buttons_pairs = [list(pair) for pair in zip(group_buttons[::2], group_buttons[1::2])]
...
def get_student_kb(group, page):
    ...
    group_buttons = [[InlineKeyboardButton(text=el['Student'],
                                           callback_data="id_student_" + str(el['id_student']))]
                     for el in student]
    ...
    if page > 1:
        end_buttons.append(InlineKeyboardButton(text="⬅️",
                                                callback_data="student_page_" + str(page - 1)))
