...
async def send_file(message: Message, state: FSMContext) -> None:
    ...
    if group_id is None:
        ...
        sent_mess = await message.answer(
            "Придется подождать какоето время, лучше никуда не нажимай"
        )
        ...
        if lessons:
            for lesson in lessons:
                ...
                day_start = year + "-8-30"
                dates = bd.get_lesson_dates(
                    lesson=lesson, group_id=group_id, date_start=day_start
                )
                # sheet.append(dates)
                rows = [dates] + (
                    bd.get_student_list(
                        dates=dates[1:], group_id=group_id, lesson=lesson
                    )
                )
                for row in rows:
                    sheet.append(row)
                ...
                for col in sheet.columns:
                    ...
                    column = col[
                        0
                    ].column_letter  # Получаем буквенное обозначение столбца (A, B, C, ...)
                    for cell in col:
                        try:
        ...
        await message.answer(
            "В файле списки присутствующих в твоей группе за текущий учебный год",
...
async def undo_button(message: Message, state: FSMContext):
    ...
    await message.answer_sticker(
        "CAACAgIAAxkBAAEMARVmLVqeD2IT8158d77O5mFlpgWr0wACyUAAAr5sEEgMQFhAO8zizTQE"
    )
...
async def echo_handler(message: Message) -> None:
    ...
    await message.answer_sticker(
        "CAACAgIAAxkBAAEMAQ5mLVcL7_veDeDVSOveH12ChgpjlAACGUIAAnn-6UgGEvxtyY9dhTQE"
    )
...
async def education(callback: CallbackQuery, state: FSMContext):
    ...
    await callback.message.answer(
        "Выбери сециальность", reply_markup=kb.get_education_kb()
    )
...
async def student_done(callback: CallbackQuery, state: FSMContext):
    ...
    await callback.message.answer_sticker(
        "CAACAgIAAxkBAAEMAw1mLoQn_rCjO8Xari_DzfzIriS1xgACjRYAAqBhwEsofH0_ZuoMbDQE"
    )
    await callback.message.answer(
        info_text, reply_markup=kb.first_kb, parse_mode="html"
    )
    await callback.message.answer_sticker(
        "CAACAgIAAxkBAAEMARFmLVqDWXJu6zTfHeHGH9Ug8Eqx6wACyUAAAr5sEEgMQFhAO8zizTQE"
    )
...
async def cmd_undo(callback: CallbackQuery, state: FSMContext):
    ...
    await callback.message.answer(
        "Ты нажал кнопку отмены, твоя авторизация сбросилась!"
    )
    await callback.message.answer_sticker(
        "CAACAgQAAxkBAAEMAwdmLoCk8K-i0bZxUXGZ1xBI7tUVWgACjwcAAvnhgFEO9a_zf5fHtDQE"
    )
    await callback.message.answer(
        info_text, reply_markup=kb.first_kb, parse_mode="html"
    )
