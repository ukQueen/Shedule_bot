    ...
    try:
        ...
        try:
            with (connection.cursor() as cursor):
                institute_select = """SELECT DISTINCT institute.id_institute, institute.Institute
FROM student
...
WHERE education.Education ='""" + education + """' AND institute.Institute LIKE '%нститут%'
...
def get_groups(fuclty, course, education):
    try:
        ...
        try:
            with (connection.cursor() as cursor):
                select_group = """SELECT DISTINCT study_group.Study_group, study_group.id_study_group
FROM student
JOIN study_group ON student.id_study_group = study_group.id_study_group 
JOIN education ON student.id_education = education.id_education 
WHERE student.course = """ + str(course) + " AND student.id_fuclty = " + str(fuclty) + """ AND 
...
def get_fuclty(institute, education):
    try:
        ...
        try:
            with (connection.cursor() as cursor):
                select_fuclty = """SELECT DISTINCT fuclty.Fuclty, fuclty.id_fuclty
FROM student
JOIN fuclty ON student.id_fuclty = fuclty.id_fuclty
JOIN education ON student.id_education = education.id_education 
JOIN study_group ON student.id_study_group = study_group.id_study_group 
WHERE student.id_institute =""" + str(institute) + """ AND 
education.Education='""" + education + """' AND 
study_group.Study_group NOT LIKE '%з%' AND study_group.Study_group NOT LIKE '%в%'
...
def get_institute_from_id(id_institute):
    try:
        ...
        try:
            with (connection.cursor() as cursor):
                select_institute = """SELECT DISTINCT Institute
FROM institute
WHERE id_institute=""" + str(id_institute) + ";"
...
def get_fuclty_from_id(id_fuclty):
    try:
        ...
        try:
            with (connection.cursor() as cursor):
                select_fuclty = """SELECT DISTINCT Fuclty
FROM fuclty
WHERE id_fuclty=""" + str(id_fuclty) + ";"
                cursor.execute(select_fuclty)
...
def get_study_group_from_id(id_study_group):
    try:
        ...
        try:
            with (connection.cursor() as cursor):
                select_study_group = """SELECT DISTINCT Study_group
FROM study_group
WHERE id_study_group=""" + str(id_study_group) + ";"
...
def get_student_from_id(id_student):
    try:
        ...
        try:
            with (connection.cursor() as cursor):
                select_student = """SELECT DISTINCT Student
FROM student
WHERE id_student=""" + str(id_student) + ";"
...
def get_student(group):
    try:
        ...
        try:
            with (connection.cursor() as cursor):
                select_fuclty = """SELECT DISTINCT student.Student, student.id_student
FROM student
WHERE student.id_study_group=""" + str(group) + " ORDER BY student.Student;"
                cursor.execute(select_fuclty)
                groups = cursor.fetchall()
...
def get_schedule_url(group_id):
    ...
    try:
        ...
        try:
            with (connection.cursor() as cursor):
                select_group = """SELECT url FROM study_group
WHERE id_study_group = """ + str(group_id) + ";"
                cursor.execute(select_group)
                url = cursor.fetchall()
...
def get_geo(spot):
    try:
        ...
        try:
            with (connection.cursor() as cursor):
                select_institute = """SELECT latitude, longitude
FROM spot
WHERE Spot='""" + str(spot) + "';"
                cursor.execute(select_institute)
                geo = cursor.fetchall()
...
def insert_lesson(lesson, group_id, student_id, year, month, day):
    try:
        ...
        try:
            with (connection.cursor() as cursor):
                lesson_insert = ("INSERT IGNORE INTO lesson (Lesson) VALUES ('" + lesson + "');")
                print(lesson_insert)
                cursor.execute(lesson_insert)
                connection.commit()

                lesson_id_select = "SELECT id_lesson From lesson WHERE Lesson = '" + lesson + "';"
                print(lesson_id_select)
                cursor.execute(lesson_id_select)
                lesson_id = cursor.fetchall()[0]['id_lesson']
                print(lesson_id)

                shedule_insert = ("INSERT IGNORE INTO schedule_list (id_lesson, " +
                                  "id_study_group, id_student, lesson_date) "
                                  "VALUES (" + str(lesson_id) + ", " + str(group_id) + ", " + str(student_id) +
...
def get_lessons(group_id):
    try:
        ...
        try:
            with (connection.cursor() as cursor):
                select_id_lesson = """SELECT DISTINCT id_lesson
FROM schedule_list
WHERE id_study_group =""" + str(group_id) + ";"
                ...
                for i in id_lessons:
                    select_lesson = "SELECT Lesson from lesson Where id_lesson = " + str(i['id_lesson']) + ";"
    ...
    try:
        ...
        try:
            with (connection.cursor() as cursor):
                ...
                select_lesson = """SELECT id_lesson
                FROM lesson
                WHERE Lesson ='""" + lesson + "';"
               ...
               # print(id_lesson)
                ...
                select = ("SELECT DISTINCT lesson_date " +
                          "FROM schedule_list " +
                          "WHERE id_study_group = " + str(group_id) + " AND id_lesson = " + str(id_lesson) +
                          " AND lesson_date > '" + date_start + "' ORDER BY(lesson_date) ASC;")
                print(select)
                ...
                for date in buf:
                    ...
                    date_normal = (str(date['lesson_date'].day) + "." +
        ...
        try:
            with (connection.cursor() as cursor):
                ...
                select = ("SELECT Student " +
                          "FROM student " +
                          "WHERE id_study_group =" + str(group_id) + " ORDER BY Student ASC;")
               ...
               # print(id_lesson)
                for student in students:
                    # print(student[0])
                    select_id_student = """SELECT id_student
                        FROM student
                        WHERE Student ='""" + student[0] + "';"
                    ...
                    for date in dates:
                        # print(date)
                        select_visiting = ("SELECT id_schedule_list FROM schedule_list WHERE" +
                                           " id_lesson = " + str(id_lesson) +
                                           " AND id_student = " + str(id_student) +
                                           " AND lesson_date = '" + str(date.split('.')[2]) + "-" +
                                           str(date.split('.')[1]) + "-" + str(date.split('.')[0]) + "';")
    ...
    try:
        ...
        try:
            with (connection.cursor() as cursor):
                ...
                delete_text = ("DELETE FROM schedule_list WHERE id_lesson= " + str(lesson_id) +
                               " AND id_study_group = " + str(group_id) +
                               " AND id_student = " + str(student_id) +
                               " AND lesson_date = '" + str(year) + '-' + str(month) + '-' + str(day) + "';")
                print(delete_text)
